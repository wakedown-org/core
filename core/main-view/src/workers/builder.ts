...
export class WorldBuilder {
  ...
  public getLayers(numPoints: number, numPlates: number = 10, plateMaxSize: number = 42, perlinFactor: number = 1 / 2, numPlateCellsInter: number = 5): Promise<GeoJson> {
    ...
    return new Promise<GeoJson>(resolve => {
      progress.start();
      const cells = Helper.GetVoronoi(Helper.GetPoints(numPoints, this.pseudo));
      const plates = Helper.GetVoronoi(Helper.GetPoints(numPlates, this.pseudo));
      ...
      cells.features.forEach((feature, idx) => {
        ...
        const distance = Helper.Distance(site, site_plate);
        let elevation = perlin * perlinFactor + Helper.Elevation(distance, plate_size);

        let biome = WorldBiome[Helper.GetBiome(elevation)];
        let cssclass = biome;
        ...
        if (between_plates.length > 1) {
          ...
          // TODO trocar aqui, das placas para as celulas vizinhas
          between_plates.forEach((pidx) => {
            if (pidx !== plate_idx) {
              const psite = <number[]>plates.features[plate_idx].properties['site'];
              const pdistance = Helper.Distance(psite, site_plate);
              const pperlin = this.getInformation(psite);
              elevationAverage += pperlin * perlinFactor + Helper.Elevation(pdistance, plate_size);
            }
          });
          elevationAverage /= between_plates.length;

          elevation = elevationAverage;
          biome = WorldBiome[Helper.GetBiome(elevation)];
